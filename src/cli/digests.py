...
def _load_discovery_issues() -> list[dict]:
    try:
        data = json.loads(DISCOVERY_ISSUES_PATH.read_text())
        return data.get("issues", [])
    except FileNotFoundError:
        return []
    except json.JSONDecodeError as exc:  # pragma: no cover - defensive logging
        logger.warning("Failed to parse discovery issues log: %s", exc)
        return []
...
def handle_daily(args) -> None:
    ...
    discovery_issues = _load_discovery_issues()
